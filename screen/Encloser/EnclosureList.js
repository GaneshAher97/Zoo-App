...
const EnclosureList = () => {
    ...
    const handelID = (id) => {
        navigation.navigate("EnclosureDetails", { itemId: id })
    }


    const renderItem = (item) => {
        // console.log(item);
        const { enclosure_id, enclosure_status, user_enclosure_name, enclosure_code, created_at, enclosure_desc, enclosure_environment, enclosure_sunlight, enclosure_lat, enclosure_long } = item;
        return (
            <TouchableOpacity style={[styles.listContainer, styles.shadow]} onPress={() => handelID(enclosure_id)}>
                <View style={styles.header}>
                    <View style={styles.innerHeader}>
                        <Text>ID : </Text>
                        <Text style={styles.idNumber}>{`#${enclosure_id}`}</Text>
                    </View>
                    <Text><Ionicons onPress={() => openMap(enclosure_lat, enclosure_long)} name="navigate" size={24} color="#00abf0" />
                    </Text>
